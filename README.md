...
By default, the server listens on `http://localhost:8000` and uses `claude-opus-5` when a request omits `model`.
...
| `-default-model` | `DEFAULT_MODEL` | `claude-opus-5` | Model alias or Bedrock model ID used when requests omit `model` |
...
Anthropic models and `gpt-oss-120b` / `gpt-oss-20b` are served by Converse. `gpt-5.5` and `gpt-5.4` are served only by the Responses API on Bedrock Mantle — there is no Converse or Invoke path for them, they have no cross-region inference profiles, and they are available in `us-east-1` and `us-east-2` only. Set `-region` accordingly if you want to use them.
