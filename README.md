...
By default, the server listens on `http://localhost:8000` and uses `claude-opus-5-5` when a request omits `model`.
...
| `-default-model` | `DEFAULT_MODEL` | `claude-opus-5-5` | Model alias or Bedrock model ID used when requests omit `model` |
...
Anthropic models, including `claude-opus-5-5`, the GPT-6 family (`gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`), and `gpt-oss-120b` / `gpt-oss-20b` are served by Converse. Claude Opus 5.5 resolves to the geographic cross-region inference profile (for example, `us.anthropic.claude-opus-5-5`); it uses adaptive reasoning, so the proxy drops unsupported sampling parameters and raises very small output-token budgets. The GPT-6 models resolve similarly to their geographic profiles (for example, `us.openai.gpt-6-sol`). Note that `gpt-6-sol` / `gpt-6-luna` are distinct from `gpt-5.6-sol` / `gpt-5.6-luna`. The GPT-5 family (`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, `gpt-5.4`) is served only by the Responses API on Bedrock Mantle — there is no Converse or Invoke path for them, they have no cross-region inference profiles, and they are in-region only: `us-east-1` and `us-east-2` for all of them, plus `us-west-2` for `gpt-5.6-terra` and `gpt-5.6-luna`. Set `-region` accordingly if you want to use them.
